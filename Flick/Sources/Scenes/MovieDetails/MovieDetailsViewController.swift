...
class MovieDetailsViewController: UIViewController {
    ...
    UICollectionViewDiffableDataSource<MovieDetailsViewModel.Section, Movie>
    typealias Snapshot =
    NSDiffableDataSourceSnapshot<MovieDetailsViewModel.Section, Movie>
    typealias MovieCollectionViewCellRegistration =
    UICollectionView.CellRegistration<MovieCollectionViewCell, Movie>
    typealias MovieDetailsHeaderRegistration =
    UICollectionView.SupplementaryRegistration<MovieDetailsHeaderView>
    ...
    private func configureDataSource() {
        ...
        let movieDetailsHeaderRegistration = MovieDetailsHeaderRegistration(
            elementKind: UICollectionView.elementKindSectionHeader
        ) { [weak self] (supplementaryView, string, indexPath) -> Void in
            guard let self = self else { return }
            supplementaryView.delegate = self
            supplementaryView.configuration = self.viewModel.movieDetails?.toMovieDetailsContentConfiguration()
        }
        
        dataSource.supplementaryViewProvider = {
            (collectionView, kind, indexPath) -> UICollectionReusableView? in
            return collectionView.dequeueConfiguredReusableSupplementary(
                using: movieDetailsHeaderRegistration,
                for: indexPath
            )
        }
